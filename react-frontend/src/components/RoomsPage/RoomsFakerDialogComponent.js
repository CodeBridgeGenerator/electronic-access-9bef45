...
import roomsFakerFactory from "./roomsFakerFactory";
...
const RoomsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = roomsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("rooms").create(elem));
...
export default connect(mapState, mapDispatch)(RoomsFakerDialogComponent);
