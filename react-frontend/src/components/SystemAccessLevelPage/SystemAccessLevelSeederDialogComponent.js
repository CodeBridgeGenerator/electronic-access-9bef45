...
import seederData from "./systemAccessLevel.seeder.json";
...
const SystemAccessLevelSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("systemAccessLevel").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(SystemAccessLevelSeederDialogComponent);
