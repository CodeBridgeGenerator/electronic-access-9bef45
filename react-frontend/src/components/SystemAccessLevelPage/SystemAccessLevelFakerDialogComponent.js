...
import systemAccessLevelFakerFactory from "./systemAccessLevelFakerFactory";
...
const SystemAccessLevelFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = systemAccessLevelFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("systemAccessLevel").create(elem));
...
export default connect(mapState, mapDispatch)(SystemAccessLevelFakerDialogComponent);
