...
import accessGroupRoomMapFakerFactory from "./accessGroupRoomMapFakerFactory";
...
const AccessGroupRoomMapFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [accessGroupIDIds, setaccessGroupIDIds] = useState();
const [roomIDIds, setroomIDIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("accessGroups")
                        .find({ query: { $select: ["_id","accessGroupID"] } })
                        .then((data) => {
                        setaccessGroupIDIds(data.data);
                        });
client.service("rooms")
                        .find({ query: { $select: ["_id","roomID"] } })
                        .then((data) => {
                        setroomIDIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = accessGroupRoomMapFakerFactory(props.user,fakerCount,accessGroupIDIds,roomIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("accessGroupRoomMap").create(elem));
...
export default connect(mapState, mapDispatch)(AccessGroupRoomMapFakerDialogComponent);
