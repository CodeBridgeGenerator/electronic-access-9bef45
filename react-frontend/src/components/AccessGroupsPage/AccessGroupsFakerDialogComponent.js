...
import accessGroupsFakerFactory from "./accessGroupsFakerFactory";
...
const AccessGroupsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = accessGroupsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("accessGroups").create(elem));
...
export default connect(mapState, mapDispatch)(AccessGroupsFakerDialogComponent);
