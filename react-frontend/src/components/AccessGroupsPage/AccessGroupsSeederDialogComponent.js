...
import seederData from "./accessGroups.seeder.json";
...
const AccessGroupsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("accessGroups").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(AccessGroupsSeederDialogComponent);
